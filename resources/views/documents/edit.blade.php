    ...
    <div class="upload-form__container">
      ...
      <article class="card">
        <div class="card__header">
          <h2 class="card__header-title">Edit Document</h2>
          <a href="/documents/{{ $document->id }}" class="upload-form__header-button">Back to details page</a>
        </div>

        <div class="card__body">
          <form action="/documents/{{ $document->id }}" class="upload-form" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PATCH')
            ...
            <div class="upload-form__line">
              ...
              @enderror
              <small class="upload-form__line-clarification">The title of the documen t being uploaded.</small>
            </div>
            ...
            <div class="upload-form__line upload-form__line--file">
              ...
              <div class="upload-form__line-input">
                ...
                <div class="upload-form__current-file">
                  <small> {{ $document->filename }} ({{ $document->file_size_human }})</small>
                </div>
                @endif
            ...
            <div class="upload-form__line upload-form__line--select">
              ...
              <select name="category_id" id="category_id"
                ...
                <option value="{{ $id }}" {{ old('category_id', $document->category_id ?? '') == $id ? 'selected' : '' }}>
                  {{ $name }}
                </option>
                @endforeach
              </select>
            ...
            <div class="upload-form__line">
              ...
              <input type="date" name="created_date" id="created_date"
                ...
                value="{{ old('created_date', $document->created_date?->format('Y-m-d') ?? '') }}">
              @error('created_date')
              <div class="upload-form__error-message">{{ $message }}</div>
              @enderror
            </div>
