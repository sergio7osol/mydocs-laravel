<x-layout :pageTitle="$pageTitle" :users="$users" :currentUserId="$document->user_id" :userDocCounts="$userDocCounts" :currentCategory="$document->category_id" :documents="$documents">
  ...
  @if(isset($document) && $document)
    ...
    <div class="document-view__content">
      @if(isset($document->filename) && pathinfo($document->filename, PATHINFO_EXTENSION) === 'txt')
        ...
        @php
          $absolutePath = env('DOCKER_ENV') === 'true'
            ? $document->file_path
            : base_path($localPath ?? $document->file_path);
        @endphp
        ...
        <div class="document-view__file-info">
          <p class="document-view__file-path">
            ...
            <span class="path-container">
              <code id="filePath"
                class="copyable-path"
                data-path="{{ $windowsFilePath ?? $document->file_path }}"
    ...
    <section class="document-actions">
      ...
      <form method="POST" action="{{ route('documents.destroy', $document) }}" class="document-actions__form" onsubmit="return confirm('Are you sure you want to delete this document?');" onclick="event.stopPropagation();">
        @csrf
        @method('DELETE')
