<x-layout :pageTitle="$pageTitle">
    <div class="content">
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="upload-form">
            @csrf
            @method('PUT')

            <div class="upload-form__line">
                <label for="name" class="upload-form__line-title">Name:</label>
                <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" class="upload-form__line-input" required>
            </div>
            ...
            <div class="upload-form__line upload-form__line--select">
                ...
                <select id="parent_id" name="parent_id" class="upload-form__line-input">
                    ...
                    @foreach ($parents as $p)
                        <option value="{{ $p->id }}" {{ (string) old('parent_id', $category->parent_id) === (string) $p->id ? 'selected' : '' }}>
            ...
            <div class="upload-form__line">
                ...
                <label class="checkbox">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', (int) $category->is_active) ? 'checked' : '' }}> Active
                </label>
            </div>

            <div class="upload-form__line">
                <label for="display_order" class="upload-form__line-title">Display Order:</label>
                <input type="number" id="display_order" name="display_order" value="{{ old('display_order', $category->display_order) }}" class="upload-form__line-input" step="1">
            </div>
